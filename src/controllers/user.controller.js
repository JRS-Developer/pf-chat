...
const getById = async (req, res, next) => {
  try {
    ...
    user
      ? res.json(user)
      : res.status(400).json({ msg: "There isn't any user with that id" })
  } catch (error) {
    next(error)
  }
}
...
const createUser = async (req, res, next) => {
  try {
    const { user, rol, email, username, fullname, avatar } = req.body
...
const deleteUser = async (req, res, next) => {
  try {
    ...
    del.deletedCount > 0
      ...
      : res.status(400).json({ msg: "doesn't exist any user with this id" })
  } catch (error) {
    next(error)
  }
}
