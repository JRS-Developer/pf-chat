...
const getClases = async (req, res, next) => {
    ...
    try{
        ...
        if(school){
            ...
            clase.length > 0 ? res.json(clase) : res.status(400).json({
...
const getClaseById = async (req, res, next) => {
    ...
    try{
            ...
            clase ? res.json(clase) : res.status(400).json({
...
const updateClase = async (req, res, next) => {
    try{
        ...
        const claseFind = await Clase.updateOne({_id: id},{
       ...
       claseFind ? res.json({msg: "clase succesfully modified"}) : res.status(400).json({ msg: "Doesn't exist any materia with that id"})
...
const deleteClase = async (req, res, next) => {
    ...
    try{
        ...
        console.log(del)
        del.deletedCount > 0 ? res.json({msg: "school succesfully deleted"}) : res.status(400).json({msg: "doesn't exist any clase with this id"})
    } catch(error){
        next(error);
    };
};
